---
title: "Explore Community Life Engagement"
path: "/projects/ecle"
date: 2026-09-04
posttype: "page"
thumbnail: "../../images/CLE-square-logo.png"
description: "A holistic approach to aligning employment and community life engagement practices, with links to the CLE Toolkit, Agency Change Toolkit, courses, and publications."
---

<div class="ecle-page">

<p class="text-center">
  <img src="../../images/ecle-logo.png" alt="Explore Community Life Engagement, UMass Boston ICI" class="img-fluid ecle-banner" />
</p>

<nav class="ecle-toplinks" aria-label="Explore Community Life Engagement quick links">
  <ul>
    <li><a href="https://cletoolkit.communityinclusion.org/" class="btn btn-outline-primary">CLE Toolkit</a></li>
    <li><a href="https://act.thinkwork.org/" class="btn btn-outline-primary">Agency Change Toolkit</a></li>
    <li><a href="https://elearning.communityinclusion.org/browse/cle" class="btn btn-outline-primary">Courses</a></li>
    <li><a href="https://cletoolkit.iciboston.org/resources/engage-briefs" class="btn btn-outline-primary">Publications</a></li>
  </ul>
</nav>

<div class="ecle-diagram">
  <h2 class="ecle-diagram-title">Holistic Agency Alignment</h2>
  <div class="ecle-diagram-body">
    <div class="ecle-diagram-toprow">
      <div class="ecle-box ecle-box-focus">
        <h3>Focus and Values</h3>
        <ul>
          <li>Clear and consistent goals</li>
          <li>Agency promotes belonging</li>
        </ul>
      </div>
      <span class="ecle-arrow ecle-arrow-h" aria-hidden="true">&#8596;</span>
      <div class="ecle-box ecle-box-infra">
        <h3>Infrastructure</h3>
        <ul>
          <li>Properly allocated resources</li>
          <li>Strong communication plan</li>
          <li>Ongoing staff development</li>
          <li>Performance measurement &amp; QA</li>
          <li>Community partnerships</li>
        </ul>
      </div>
    </div>
    <div class="ecle-diagram-arrowsrow" aria-hidden="true">
      <span class="ecle-arrow ecle-arrow-v">&#8597;</span>
      <span class="ecle-arrow ecle-arrow-v">&#8597;</span>
    </div>
    <div class="ecle-box ecle-box-practices">
      <h3>Employment &amp; Community Life Engagement Practices</h3>
      <ul class="ecle-practices-list">
        <li>A holistic approach</li>
        <li>Customer focus &amp; engagement</li>
        <li>Active person-centered job development</li>
        <li>Promotes community membership</li>
        <li>Outcome-oriented &amp; monitored supports</li>
        <li>Develop relationships &amp; build skills to reduce reliance on paid staff</li>
      </ul>
    </div>
  </div>
  <p class="visually-hidden">Focus and Values, Infrastructure, and Employment &amp; Community Life Engagement Practices are interconnected and reinforce one another.</p>
  <div class="ecle-diagram-footer py-3">
    <div class="ecle-footer-logo">
      <img src="../../images/thinkwork-update.png" alt="ThinkWork! UMass Boston ICI" />
    </div>
    <div class="ecle-footer-logo">
      <img src="../../images/ecle-logo.png" alt="Explore Community Life Engagement" />
    </div>
  </div>
</div>

<div class="ecle-text">

## What is a holistic approach?

A holistic approach looks at **the whole person**, not just one outcome.

## Align your agency's work

What can you do to make sure everyone is on the same page?

- Create a shared vision and shared goals
- Look at how your agency functions to make sure everyone is working together
- Make a commitment to employment, community life engagement, and opportunities for all people

## Why does a holistic approach work so well?

By intentionally working together, you can:

- make the most of limited resources
- avoid doing the same work twice
- help people with disabilities to get what they need

</div>

<div class="row ecle-cards mt-3">
  <div class="col-md-6 mb-4 d-flex align-self-stretch">
    <a href="https://act.thinkwork.org/" class="card shadow-sm p-3 text-center ecle-card">
      <p>Are you ready to make a commitment to holistic agency alignment?<br />
      <span class="ecle-card-cta">Explore the Agency Change Toolkit!</span></p>
      <img src="/images/agencychange-subdued.svg" alt="" class="img-fluid ecle-card-logo" />
    </a>
  </div>
  <div class="col-md-6 mb-4 d-flex align-self-stretch">
    <a href="https://cletoolkit.communityinclusion.org/" class="card shadow-sm p-3 text-center ecle-card">
      <p>Are you ready to improve meaningful community life engagement for the people you support?<br />
      <span class="ecle-card-cta">Explore the CLE Toolkit!</span></p>
      <img src="/images/cle-toolkit-logo.png" alt="" class="img-fluid ecle-card-logo" />
    </a>
  </div>
</div>

</div>
